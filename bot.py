...
class DerivTradingBot:
    ...
    async def place_trade(self, direction):
        ...
        expiration_time = self.last_trade_time + timedelta(minutes=self.contract_duration)
        logger.info(f"Placed {direction} trade for {self.stake_amount} USD")

        if self.gui_callback:
            self.gui_callback(f"Placed {direction} trade for {self.stake_amount} USD at {self.last_trade_time.strftime('%Y-%m-%d %H:%M:%S')} with expiration at {expiration_time.strftime('%Y-%m-%d %H:%M:%S')}")
